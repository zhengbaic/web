var start, puzzles, is_start, check_num, clear;//check_num是放位置的数组 也是检测位置的标准
/*提取数字*/
var which_num = {
    one:1,
    two:2,
...
var init = function () {
    ...
    for(var i = 1; i < 16; i++) {
        var o = document.getElementById(which_letter[i]);
        o.className = "puzzle "+ which_letter[i];
    }
    var p = document.getElementById("sixteen");
    p.className = "blank sixteen";
};
/*打乱*/
var run = function () {
    is_start = true;
...
window.onload = function () {
    ...
    for (var i = 0; i < puzzles.length; i++) {
        ...
        puzzles[i].ondblclick = function () {
            ...
            var that = this;
            if (is_start == false) return;
            game_box.className = 'double_click';
