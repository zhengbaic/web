...
var stop; //使完全停止
var xhr = [];
var CurrentButton = 0;//当前获取随机数的按钮
var Order;//获取随机数的按钮顺序
var Robot;//是否开始机器人

$(function() {
    $('.apb').on('click', AiInOrder);
...
function ButtonClickHandler(e) {
    ...
    xhr[letter[e.target.id]] = $.get("GetRandonmNumber", function (data) {
        ...
        if(Robot) {
            CurrentButton++;
            if(CurrentButton < 5) {
                $($('.button')[Order[CurrentButton]]).trigger('click');
            } else {
...
function AiInOrder(e) {
    InitStaus();
    Order = [0,1,2,3,4].sort(()=>Math.random() - 0.5);
    var letter_order = Order.map(function (x) {
        return num[x];
    });
    $("#order").html("order: " + letter_order.join(" "));
    Robot = true;
    CurrentButton = 0;
    $($('.button')[Order[CurrentButton]]).trigger('click');
}
